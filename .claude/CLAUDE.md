...
| Fetch an official PDF form (any institution) over https with the validated ingest contract (scheme re-checked on every redirect hop, 25 MiB cap enforced mid-stream, `%PDF` magic bytes, 30 s timeout, atomic write) and report its SHA-256. Holds no catalogue: which forms exist, and whether one changed, is ThesisTracker's record (TT-8), not this skill's | `form-service` skill | `/fetchform` |
| Set up, tune, or run the aider nightly local-coding pipeline (no Claude Code involved once the machine is set up: one aider process per plan, a local writer model that codes and tests, a local reviewer model that never edits, driven overnight by `aider-night.ps1` and gated by measured token budgets), or rebuild the distribution zip handed to students | `aider-setup` skill | by name; `.claude/skills/aider-setup/scripts/aider-night.ps1` for the run itself |
| Read or search the Obsidian vault (notes, tags, tasks, links, properties), or deposit a captured learning for it (new or appended content routes through the outbox only, and `--apply --yes` link maintenance is the one in-place exception, run by the same serialized writer); a fixed queue under the same outbox (`ask/requests/`, `ask/answers/`) additionally answers a question, gated to requests declaring `from: rt-dashboard` - see the `rt-observe` row's voice panel | `obsidian-cli` skill, reached ONLY through the `local-writer` agent, which is the sole agent with vault access, reading included. `vault-access-guard.py` refuses every other caller at the tool boundary | dispatch `local-writer` |
...
| Check that a citing sentence says what the cited paper actually claims, rather than what its abstract is about. Mode `validate` pairs every `\cite{}` of a manuscript with its paper's own contribution sentences; mode `mine` tabulates a whole `refs/` corpus. The script supplies the evidence VERBATIM and decides nothing: `kinds_found` is a sorting aid and never a verdict, and the four statuses (`ok`, `no-contribution`, `empty`, `unreadable`) are never merged, since a one-page preview read as a silent paper blames the article for a retrieval failure. Its markers are data in `contribution_markers.json`, extended rather than the module - measured 2026-09-13, a catalogue written in the engineering idiom reported a planning paper that states its contribution four times as stating none. Used inside the four auditors (`validate` mode, checking a finished manuscript's `\cite{}` sentences against the papers it cites) and inside `scopus-researcher` (`mine` mode, Step 3b-CIT), `litreview-updater` (`mine` mode, Step 5) and `reviewer-response` (single-paper mode, Step 6c) so a NEW citing sentence is authored from a paper's own downloaded contribution rather than from its abstract; the shared citing rule lives once in the `scientific-writing` skill's `citation_styles.md` override | `extract-contributions` skill | by name |
| Extract a paper's own content into one JSON, then draft or refresh its abstract (or, for a UQAC thesis, its Resume + Abstract pair) from that JSON, grounded in the paper's own contribution/novelty/method/result sentences rather than a paraphrase - never citations, never bilingual for a paper, always both languages for a thesis | `extract-paper-idea` skill, `abstract-writer` agent | `/abstract` |
...
| Draft, refresh, or tailor the narrative "CV descriptif" to one grant competition (FRQ's CV-FRQ, structurally identical to the tri-agency CIHR/NSERC/SSHRC CV commun des trois organismes: three sections, up to 10 items in section 2, 6-page FR / 5-page EN cap). Built around a durable master contributions inventory in the researcher's own external project folder (`cv.project_dir` in the active profile), refreshed via Scopus + `extract-contributions` and re-ranked per competition rather than rebuilt from scratch each time; drafted through `scientific-writing`; renders LaTeX/PDF (old-portal Times New Roman, tri-agency Helvetica-substituted-Arial) and a plain-text companion for the new-portal paste-in channel from one JSON model | `narrative-cv` skill / `narrative-cv-writer` agent | `/cv` |
| Ask whether this toolkit is correctly deployed to every harness, and which empty mirror cells are DELIBERATE rather than lost. The mirror matrix compares canonical definitions against every dialect `install.ps1` generates, reading intent from `mirror-policy.json` so the by-design/lost distinction survives on a machine with no PowerShell. It also reports the definition set's own integrity, the green stamp and active profile, plan progression from `PROGRESS.md` cross-checked against the plan's own phase headings, the MCP roster in two tiers (live only when the `claude` binary is present), local model residency, the vault daemon and its queue depth, and sessions from the Claude Code and Copilot Chat adapters. Standard library only, no harness required, and zero harnesses is a supported configuration. It never reads the vault, and it renders the code graph ONLY from a snapshot `local-writer` produced - a server reading the graph on your behalf is the bypass `vault-access-guard.py` exists to stop. Since 2026-09-24 it also reports an OPTIONAL journal - PostgreSQL identity/account-to-container mapping (`rt_store.py`) and an OpenObserve trace/audit stream (`rt_openobserve.py`, a second sink beside the existing JSONL action log) - absent by default, reporting `unavailable` with a named reason on a clone with neither configured, never a blank panel | `rt-observe` skill | `/rt-dashboard`, or `rt-dashboard.ps1` / `.sh` / `.bat`, or `python .claude/skills/rt-observe/scripts/rt_state.py [--json]` |
| LOOK at that state rather than read a dump, and ACT on it. `rt-dashboard` serves the same snapshot as a page on 127.0.0.1, TABBED and fitting one screen with no page scroll: four views on the left - the mirror matrix as one grid per definition kind, a fan-out wiring diagram, the sessions strip, and a Real-Time Process tab drawing the harness state machine, each session's recent steps with its subagents hanging off it, which calls left the machine, and a token total whose percentage is refused because no transcript reports the window it sits in - and the rail's seven receipt-bearing panels as tabs on the right, with the plan timeline and the optional journal (identity, traces) among them. Hover detail is ONE layer for every object rather than a tooltip per panel, which is why the fan-out edge reading `1 lost` now names which mirror, and the diagram's boxes drag with their edges following. The rail also runs a closed whitelist of actions held as data in `actions.json` - an id maps to a FIXED argv, so nothing from a request reaches a command line, every id points at a script this repository already tests, a dry run resolves the argv and spawns nothing, a destructive one arms behind its own confirm sentence, and the result is judged by re-collecting the section the action claims to change rather than by its exit code. A session card can also be sent a message, which is written to `~/.claude/rt-inbox/` and delivered by a `UserPromptSubmit` hook at that session's next turn, or reported unreachable when no hook is installed - never as delivered. Loopback only, a session token on the one write route, per-section TTLs so a page poll never re-runs `claude mcp list`, and a section not yet collected reads `collecting` rather than blank. The three refusals are the reason it is a launcher and not four lines of shell: no interpreter found names every candidate it tried and exits 2, a port held by another process names the holding PID and exits 1 rather than binding a second port (two dashboards showing two different snapshots is worse than none), and an already-running dashboard is reported with its URL rather than started twice. It needs no Claude Code - the slash command is a convenience over a plain command, which is the point on a machine that runs Codex or Continue instead; a push-to-talk voice panel (hold P) transcribes a spoken question locally and relays it to the vault daemon's ask queue for an answer using vault search plus this dashboard's own state, spoken back through the browser's TTS, with a language dropdown (auto/en/fr) next to the panel | `rt-observe` skill | `/rt-dashboard`, `rt-dashboard.ps1 -DryRun` |
