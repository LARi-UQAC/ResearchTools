...
This step is autonomous (no user pause). The full protocol (debate rounds, canonical arbitration
table, provenance markers, Scopus validation gate, Deliberation-Log format) lives in
...
2. Gather Consensus counter-evidence: up to 4 targeted `mcp__claude_ai_Consensus__search` queries on
   the paper's main claims and weakest findings (batches <= 3, one query per second); write the
...
5. Merge accepted suggestions into the appropriate plan sections, then append a `## Deliberation Log`
   ...
   Conflicts resolved / Rejected lists).
...
### Step 10 — ScholarEval skill scoring (final score report)

Run this step **last**, after the Step 9 plan file is written, so the score reflects the complete audit. Where Step 8.5 produces an inline estimate (Section O), this step invokes the `scholar-evaluation` skill itself — through its `calculate_scores.py` script — to produce the authoritative, standalone score report.

1. **Map the eight Section O dimension scores to the skill's JSON keys:**

| Section O dimension | Skill JSON key |
|---|---|
| D1 — Problem Formulation | `problem_formulation` |
| D2 — Literature Review | `literature_review` |
| D3 — Methodology | `methodology` |
| D4 — Data Collection | `data_collection` |
| D5 — Analysis & Interpretation | `analysis` |
| D6 — Results & Findings | `results` |
| D7 — Scholarly Writing | `writing` |
| D8 — Citations & References | `citations` |

2. **Write the scores JSON** alongside the source file as `<source_basename>_scholareval_scores.json` (or `scholareval_scores.json` in the cwd for pasted text). Each value is the 1–5 score already assigned in Section O; the script rejects any value outside 1–5:
...
3. **Run the skill's calculator** (canonical ScholarEval weights and quality bands, so the result is reproducible and independent of the Step 8.5 hand arithmetic):
...
The report contains the overall weighted score (/5), the quality level, an ASCII bar chart of the eight dimensions, the per-dimension weighted contributions, top strengths, areas for improvement, and a recommendation line.

4. **Reconcile with Step 8.5.** If the script's overall score differs from the Section O weighted total by more than 0.05, the script is the source of truth (canonical weights) — correct the Section O value in the plan to match.

5. **Append a pointer to the plan file** so the final score is discoverable from the plan:

```markdown
## Final ScholarEval Score (scholar-evaluation skill)
Standalone report: `<source_basename>_scholareval_report.txt`
Overall: N.NN / 5.00 — [Quality level]
Generated by the `scholar-evaluation` skill (`calculate_scores.py`, canonical weights).
```

6. **Report to the user** the overall score, the quality level, and the two output paths (`..._scholareval_scores.json`, `..._scholareval_report.txt`).
