# Fetch an official PDF form

Thin wrapper over the `form-service` skill. Read
`.claude/skills/form-service/SKILL.md` and follow its workflow.

This skill holds no catalogue. It cannot tell you which forms exist, nor
...
1. Resolve the intent from `$ARGUMENTS`. Expect an https URL and, optionally, a
   ...
   meaningless digest.
2. Default the destination to `out/form-service/<basename>.pdf` and create the
   directory if needed. Everything this skill writes belongs under `out/`.
3. Run:
   ```
   python ".claude/skills/form-service/scripts/pdf_ingest.py" "<url>" "<dest>"
   ```
4. Report the outcome from the JSON on stdout:
   - `{"ok": true, ...}` and exit 0: give the path and the `sha256`, and say
     ...
     this skill's.
   - `{"ok": false, ...}` and exit 1: nothing was written. The `[FORM-SERVICE]`
