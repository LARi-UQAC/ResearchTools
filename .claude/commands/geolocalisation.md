# Map a review corpus's study locations
...
1. Resolve the corpus `.bib` path from `$ARGUMENTS` (a path, a directory containing one `.bib`, or the file open in the IDE). Choose an output directory (default: next to the `.bib`).
