# Word → LaTeX template conversion
...
1. Resolve the `.docx` path from `$ARGUMENTS` (a path, a directory containing one `.docx`, or the file open in the IDE).
...
4b. **Only if `--bibliography` is in `$ARGUMENTS`** (manuscript mode; skip for form gabarits): run a second pandoc pass to markdown, extract the numbered bibliography, and rewrite inline citations.
   ```
