---
description: "LaTeX hygiene check"
---
...
1. Resolve the target `.tex` file(s) from `the file(s) or topic given after the command in the chat message (if none was given, use the file currently open in the editor)` (a path, a glob, a `sections/*.tex`
   directory, or the file open in the IDE). Resolve the sibling `.bib` file when `citecov` is
   needed.
2. If `the file(s) or topic given after the command in the chat message (if none was given, use the file currently open in the editor)` names one check (`chars`, `aiscan`, `wc`, `abstract`, `braces`, `par`,
...
6. If `the file(s) or topic given after the command in the chat message (if none was given, use the file currently open in the editor)` names `patch` with a `--plan <audit_plan.md>` and a `--target <file.tex>`,
...
7. After a `patch` (or any manual edit), run the post-write guard before declaring the `.tex`
   ...
   ```
8. If `the file(s) or topic given after the command in the chat message (if none was given, use the file currently open in the editor)` names `accept`, resolve the tracked source to the accepted text
   ...
   ```
9. If `the file(s) or topic given after the command in the chat message (if none was given, use the file currently open in the editor)` names `build`, run the pdflatex/bibtex build sequence on the tracked or
