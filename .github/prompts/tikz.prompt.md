...
**Exception — drawio2tikz output:** a figure generated by the `drawio2tikz` skill (a coordinate-exact `.drawio` conversion) uses absolute coordinates and inline colours by design. Do not flag rule 5 (anchoring) or rule 6 (TiKZiT compatibility) for it; still check rules 1-4 (node line breaks, backgrounds + scaling, arrow angles, overlaps).
