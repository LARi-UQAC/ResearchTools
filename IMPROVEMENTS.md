...
**Also fixed while measuring:** the repository's own `.claude/CLAUDE.md` references the live
`graphify-out/` graph without explicitly routing it through `local-writer` as the rule requires,
and the `.claude/rules/testing.md` inventory listed both `check-graph-health.ps1` and
`verify-graph-health.ps1` as non-gated. Both are now marked LOCAL-WRITER ONLY with dispatch
noted. See the 2026-08-30 vault-access-guard entry for the full context and the guard itself.

**Measured 2026-09-25 against what the instrument knew then:** `.rt-mirrors.json` (previous
snapshot baseline), before the README split into 12 chapters, no change to mirror generation and
no new lost cells beyond the expected Copilot 6 (unchanged) and VS Code 7 (unchanged). The README
restructure is documentation-only; it did not touch the mirror set or its generator.

## 2026-09-26 — docs restructure: README split into chapters; graphify semantic pass deferred

**Deferral:** README.md (110 KB flat file, 1306 lines) was restructured into a ~62-line
quickstart/TOC + 12 numbered chapters under docs/manual/ (00-purpose through 11-file-locations),
mirroring Architecture.md's layer split. Graphify AST-only refresh completed at repository root
with 9011 nodes, 13007 edges, 690 communities. Semantic pass for the 13 markdown docs deferred
(cost not justified for a docs-only reorg; AST structure preserved). Note: test_graph_routing.py
